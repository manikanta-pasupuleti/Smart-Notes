...
const fetchByCategory   = cat                     => req(`${API}/category/${encodeURIComponent(cat)}`);
...
async function applyFilter() {
    ...
    if (activeCategory === 'all') {
        ...
        try {
            const filtered = await fetchByCategory(activeCategory);
            renderNotes(filtered);
        } catch {
            const filtered = allNotes.filter(n => (n.category || 'General') === activeCategory);
            renderNotes(filtered);
        }
    }
}
...
exportBtn.addEventListener('click', async () => {
    try {
        const resp = await fetch(`${API}/export`, { headers: getAuthHeaders() });
